# Privacy Policy

**ELIPHASx (Pty) Ltd**  
**Last Updated:** December 2024

---

## 1. Introduction

ELIPHASx (Pty) Ltd ("we", "us", "our") is committed to protecting your personal information in accordance with the Protection of Personal Information Act 4 of 2013 ("POPIA") and other applicable South African laws.

This Privacy Policy explains how we collect, use, store, and protect your personal information when you use our jewelry manufacturing quote management software ("Service").

---

## 2. Information Officer

**Responsible Party:** ELIPHASx (Pty) Ltd  
**Information Officer:** [Your Name]  
**Email:** privacy@eliphasx.co.za  
**Address:** [Your Business Address], South Africa

---

## 3. Information We Collect

### 3.1 Account Information
- Email address
- Password (encrypted)
- Company name
- Role within organization

### 3.2 Business Information
- Client names and contact details
- Quote data (materials, pricing, specifications)
- Metal and stone pricing configurations

### 3.3 Payment Information
- Billing address
- Payment method (processed by Paystack - we do not store card details)
- Transaction history

### 3.4 Technical Information
- IP address
- Browser type and version
- Device information
- Usage logs

---

## 4. Purpose of Processing

We process your personal information for the following purposes:

| Purpose | Legal Basis |
|---------|------------|
| Provide our Service | Contract performance |
| Process payments | Contract performance |
| Send service notifications | Legitimate interest |
| Improve our Service | Legitimate interest |
| Comply with legal obligations | Legal compliance |
| Respond to support requests | Contract performance |

---

## 5. How We Protect Your Information

We implement appropriate technical and organizational measures:

- **Encryption:** All data transmitted using TLS/SSL
- **Password Security:** Passwords hashed using bcrypt (12 rounds)
- **Access Control:** Role-based access to data
- **Rate Limiting:** Protection against brute-force attacks
- **Account Lockout:** Automatic lockout after 5 failed login attempts
- **Regular Backups:** Daily encrypted backups

---

## 6. Data Retention

| Data Type | Retention Period |
|-----------|-----------------|
| Account data | Duration of service + 30 days after deletion |
| Quote data | 5 years (tax/audit requirements) |
| Payment records | 5 years (financial regulations) |
| System logs | 90 days |

---

## 7. Your Rights Under POPIA

You have the right to:

1. **Access:** Request a copy of your personal information
2. **Correction:** Request correction of inaccurate information
3. **Deletion:** Request deletion of your personal information
4. **Object:** Object to processing of your information
5. **Restriction:** Request restriction of processing
6. **Data Portability:** Receive your data in a portable format
7. **Withdraw Consent:** Withdraw consent at any time

To exercise these rights, email us at privacy@eliphasx.co.za

---

## 8. Third-Party Service Providers

We share information with:

| Provider | Purpose | Location |
|----------|---------|----------|
| Paystack | Payment processing | South Africa |
| AWS | Cloud hosting | Cape Town, South Africa |
| SendGrid | Email delivery | USA (GDPR compliant) |

All providers are required to protect your information in accordance with POPIA.

---

## 9. Cookies

We use essential cookies for:
- Session management
- Security (CSRF protection)
- User preferences

We do not use tracking or advertising cookies.

---

## 10. International Data Transfers

Your data is primarily stored in South Africa (AWS Cape Town region). If data is transferred outside South Africa, we ensure adequate protection through:
- Standard contractual clauses
- GDPR-compliant providers

---

## 11. Children's Privacy

Our Service is not intended for individuals under 18 years of age. We do not knowingly collect information from children.

---

## 12. Changes to This Policy

We may update this Privacy Policy periodically. We will notify you of significant changes via email or in-app notification.

---

## 13. Complaints

If you are not satisfied with our response to a privacy concern, you may lodge a complaint with:

**Information Regulator (South Africa)**  
Email: inforeg@justice.gov.za  
Website: https://www.justice.gov.za/inforeg/

---

## 14. Contact Us

For privacy-related inquiries:

**Email:** privacy@eliphasx.co.za  
**Phone:** [Your Phone Number]  
**Address:** [Your Business Address], South Africa
